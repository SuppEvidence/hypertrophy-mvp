...
check("base-classified EDT clusters do not drive load progression", () => assert.equal(detect(set({ isEdt: true, setTypeId: "edt", reps: 20 })).shouldCheck, false));
check("EDT symptoms can still trigger a protective review", () => assert.equal(detect(set({ isEdt: true, pain: true })).shouldCheck, true));
