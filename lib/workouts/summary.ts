...
export type LoggedSetForSummary = {
  ...
  // Legacy fields may still exist on historical Prisma rows but are not needed.
...
export function buildWorkoutSummary(args: {
  ...
  let completedSets = 0;
  // Retained property name for compatibility. This now equals counted completed
  // set rows; manual effort labels no longer determine whether a set counts.
  ...
  function addVolume(
    item: LoggedExerciseForSummary,
    completed: number,
    effectiveEquivalent: number,
  ) {
    for (const link of item.exercise.primaryMuscles) {
      ...
      row.effective += effectiveEquivalent;
  ...
  for (const item of args.sessionExercises) {
    const setPainFlag = item.sets.some(
      (set) => set.isCompleted && Boolean(set.painFlag),
    );
    ...
    addVolume(
      ...
      contribution.productiveEquivalent,
    );
