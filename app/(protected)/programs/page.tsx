...
export default async function ProgramsPage() {
  const programs = (await listUserPrograms()) as ProgramListItem[];
    ...
    <div className="space-y-5">
      ...
      {programs.length === 0 ? (
        <Card className="space-y-4">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            {programTypeOptions.map((option: { value: ProgramType; label: string }) => (
      ...
      <div className="space-y-3">
        {programs.map((program: ProgramListItem) => {
          const priorityMuscles = program.priorityMuscles.map((link: { muscle: Pick<ReferenceMuscle, "name"> }) => link.muscle.name).join(", ") || "—";
          const targetCount = program.volumeTargets.filter((target: { weeklyTargetSets: unknown }) => Number(target.weeklyTargetSets) > 0).length;
          return (
