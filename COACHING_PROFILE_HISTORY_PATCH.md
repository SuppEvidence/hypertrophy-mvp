...
- Each exercise can have a private, athlete-specific coaching profile: preferred, neutral or avoid for new suggestions; whether new advanced set types are allowed; and short coaching notes. Profiles on seed exercises do not change the shared catalog. Editing a seed exercise into a personal copy carries its profile to the copy.
- EDT set types can appear in the profile's optional advanced set types even when marked as base sets in Settings. The choice must be explicit before T2 can introduce EDT on a supported isolation movement. EDT is still counted using its configured effective-set multiplier and does not gain an `isIntensifier` flag; legacy intensifier totals stay unchanged. Aggregate EDT cluster reps do not enter the straight-set load/rep performance comparison in T1 or subsequent coaching evidence.
