...
import { ExercisePainLogger } from "@/components/workouts/ExercisePainLogger";
...
export function ExerciseCollapseCard({
  exerciseId,
  title,
  exerciseName,
  plannedSets,
  ...
  const initialState = useMemo(
    () => new Map(initialSets.map((set) => [set.id, set.isCompleted])),
    [initialSets],
  );
  ...
  const [isOpen, setIsOpen] = useState(
    !(initialSets.length > 0 && initialCompletedCount === initialSets.length),
  );

  useEffect(() => {
    completionState.current = new Map(
      initialSets.map((set) => [set.id, set.isCompleted]),
    );
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener(WORKOUT_SET_COMPLETION_EVENT, handleCompletion);
  }, []);

  const allComplete =
    initialSets.length > 0 && completedCount === initialSets.length;

  return (
    <section
      className={`rounded-2xl border bg-slate-950 p-3 transition ${
        allComplete ? "border-emerald-500/30" : "border-slate-800"
      }`}
    >
      <button
        type="button"
      ...
      <div
        id={`exercise-panel-${exerciseId}`}
        className={isOpen ? "mt-3 block" : "hidden"}
      >
        <ExercisePainLogger sessionExerciseId={exerciseId} />
        {children}
      </div>
    </section>
  );
