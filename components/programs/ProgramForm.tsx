import type { Muscle, Program, ProgramPriorityMuscle, MuscleVolumeTarget } from "@prisma/client";
...
type EditableProgram = Program & {
  priorityMuscles: Array<ProgramPriorityMuscle & { muscle: Muscle }>;
  volumeTargets: Array<MuscleVolumeTarget & { muscle: Muscle }>;
};
...
export function ProgramForm({ muscles, program, action, defaultSecondaryContribution = 0.5 }: Props) {
  ...
  const targetMap = new Map(program?.volumeTargets.map((target) => [target.muscleId, Number(target.weeklyTargetSets)]) ?? []);
    ...
    <form action={action} className="space-y-4">
      <Card className="space-y-4">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field
            ...
            max={1}
            step="0.05"
            defaultValue={program ? Number(program.secondaryContribution) : fallback.secondaryContribution}
            required
          />
      ...
      <Card>
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-slate-400">Targets are stored now; dashboard calculations come in a later slice.</p>
        </div>
