...
export function ExerciseCoachingProfileForm({ profile, setTypes, action }: {
  profile: Profile;
  setTypes: Array<{ id: string; name: string; isIntensifier: boolean }>;
    ...
    <Card>
      <form action={action} className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="space-y-1 text-xs text-slate-400">New intensifier suggestions
            <select name="intensifierPreference" defaultValue={profile?.intensifierPreference ?? "DEFAULT"} className={inputClass}>
              ...
              <option value="NONE">No new intensifiers</option>
              <option value="ONLY_SELECTED">Only selected types below</option>
            </select>
          </label>
        </div>
        <fieldset className="space-y-1">
          <legend className="text-xs text-slate-400">Allowed types when “Only selected” is chosen</legend>
          <div className="flex flex-wrap gap-2">
            {setTypes.filter((type) => type.isIntensifier).map((type) => (
          ...
          <p className="text-xs text-slate-500">Exercise safety rules still apply. Selecting a type never authorizes it on an unsuitable movement.</p>
        </fieldset>
